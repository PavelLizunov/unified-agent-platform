...
## Практики, снятые с sister-ноды (harvest)

Разбор read-only снимка соседней hermes-ноды дал три практики прямо для роя. Полный adopt-list
(с REJECT-частью) - `docs/templates/harvest-from-hermes-nastya.md`. Кратко:

- **Дисциплина верификации диспетча.** Прежде чем просить человека перезапустить большую миссию,
  докажи РЕАЛЬНЫЙ путь диспетча маленькой one-shot smoke-миссией через тот же API и проверь
  структурный чекпоинт (HTTP 200, exit 0, статус чекпоинта, `FILES_CHANGED: none`). Зелёная сборка
  НЕ доказывает, что воркеры спавнятся. У нас это уже частично есть - healthcheck обоих эндпоинтов
  роутера перед `kanban swarm` (см. блок с warning выше). На Windows-без-tmux рой может уйти в
  one-shot fallback - проверяй фактический путь, не считай воркеров персистентными по умолчанию.
- **`tool_loop_guardrails` с `hard_stop_enabled: true`.** Конфиг-блок Hermes ловит залипшие
  тул-циклы (повтор точной ошибки / одного тула / idempotent-no-progress) в два порога:
  `warn_after` и `hard_stop_after`. Sister-нода везёт его с `hard_stop_enabled: false` (только
  warnings) - **у нас надо включить `true`**. Пороги (её же): warn 2/3/2, hard_stop 5/8/5. Это
  детерминированный stop-condition под наш watchdog: конфиг-хардстоп ловит тугой цикл, а
  `tools/swarm/watchdog.sh` - блуждающего орфана по wall-clock (находка №2 SIM-1).
- **Гоча портов gateway.** Все профили по умолчанию слушают api_server на ОДНОМ порту 8642 -
  второй gateway падает с `Port 8642 already in use`. Webcenter на build-1 уже занимает :8642
  (gateway API). Поднимаешь второй профиль/gateway на том же хосте - задай ему СВОЙ api_server-порт,
  Telegram-токен и allowlist держи в scope профиля, и НИКОГДА не гоняй два gateway на одном
  Telegram-токене (второй поллер молча ворует апдейты). Install-wide singleton-lock диспетчера -
  это by design, не баг: полагайся на него (`--replace` / gateway.lock), а не на PID-киллер-скрипты.
