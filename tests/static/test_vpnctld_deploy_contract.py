...
class VpnctldDeployContractTests(unittest.TestCase):
    ...
    def test_protocol_is_fixed_to_exact_revision_target_and_installed_payload(self):
        ...
        self.assertEqual(3, installer.count("cleanup_deploy_residue"))
        self.assertIn("find /opt/vpnctl -mindepth 1 -maxdepth 1 -type d", installer)
        self.assertIn("-name '.uap-rollback.*' -exec rm -rf -- {} +", installer)
