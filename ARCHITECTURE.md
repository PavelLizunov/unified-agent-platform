...
- история/чекпойнты задачи → журнал Restate (лог+RocksDB+S3-снапшоты); прикладные данные/учёт → Postgres;
...
> **Обновлено (2026-06-23, пивот):** «Hermes» теперь различаем — **hermes-agent** (внешний NousResearch харнесс,
> ПРИНЯТ как магистраль, ADR-024) vs **Hermes-legacy** (наш bespoke `hermes/hermes.py`, припаркован). Пост-пивот:
> харнесс = hermes-agent (gateway, телефон/Telegram, скиллы, память, субагенты); мозг = Codex `codex_app_server` /
> локальная FC-модель (ADR-025); кодинг = `claude -p` + `codex exec`. Текст ниже — исторический контекст проектирования.

- **Hermes Agent** (Nous Research) — открытый агент-рантайм; в текущей магистрали это **gateway/харнесс** (а не
  «то, что крутится внутри воркера»): принимает идею с любого устройства и раздаёт движкам.
