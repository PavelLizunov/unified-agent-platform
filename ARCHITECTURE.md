...
- **Локальные модели — дешёвый overflow для простых задач** ради экономии токенов Claude
  ...
  уходит в облако.
- **Failover** через `order=`/fallback-цепочки LiteLLM: бэкенд упал (connection error / 429 / 404) →
...
[ Tailscale mesh ] ── заходишь с Win/Mac/Linux, всё достижимо по имени
   ...
   всё ходит в LiteLLM; durable-состояние → Restate + Postgres + MinIO + git
```
...
| MinIO | на старте single-node + offsite backup; distributed-режим позже, когда ясна конфигурация дисков |
...
- рабочие артефакты → MinIO (S3-совместимо);
