...
Use [`handy-computer/gigaam-v3-e2e-rnnt-gguf`](https://huggingface.co/handy-computer/gigaam-v3-e2e-rnnt-gguf)
at repository commit `f719d70812344f4d0fb8c11c0887b190501a7465`, file
`gigaam-v3-e2e-rnnt-Q4_K_M.gguf`, with the CPU build of
[`handy-computer/transcribe.cpp`](https://github.com/handy-computer/transcribe.cpp) release `v0.1.3`.
It runs as one bounded command inside the existing Hermes pod. It is not a service, agent, author, reviewer or
...
|---|---|---|
| Source identity | repo commit `f719d70812344f4d0fb8c11c0887b190501a7465` | repo `main` commit `ec1dc1f01d0d627ab2c0d3acc1e235702300d95e`; `e2e_rnnt` revision `7655ad717f8122257385bb4b2f373db3697e8680` |
| License | MIT metadata; conversion points to the MIT upstream | MIT metadata and upstream repository license |
| Selected/nearest weight | Q4_K_M: 183,948,704 bytes, SHA-256 `7d69952fb431a8d7800ed9910dc61fea37d8406bfe96d10bf24c8bd4b7c68623` | `pytorch_model.bin`: 448,928,167 bytes, SHA-256 `afc6dcbae8320ea56f2cddebc0f13fbf62c9d59b6ddcad899782623c8610826a` |
...
evidence. The production choice is based on the checked files, exact hashes and the live CPU run below.
...
|---|---|---|
| `uap-home-2` | Debian 12 VM, 6 vCPU with SSE4.2, 8 GB RAM, no GPU; Hermes pod limit 4 GiB | selected production host |
...
| owner Mac mini | M4/16 GB | always-on but already carries Ornith; unnecessary |
...
Q8 and GPU execution were not advanced after Q4 passed the accuracy and latency canary. Q8 would add about 90 MB of
model storage without closing a demonstrated problem; GPU would add an unavailable production dependency.
...
- FFmpeg `-nostdin`, one decode thread, no video/subtitle/data, 16 kHz mono float PCM, 12-second decode timeout and
  bounded PCM output;
- CPU backend, two inference threads, 40 CPU seconds, 1.5 GiB address-space ceiling, 64 file descriptors, a
  45-second outer command timeout and an exclusive model lock;
