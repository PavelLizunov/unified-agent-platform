...
def run_react(user_text, model=None, reasoning_effort=None, max_steps=MAX_STEPS):
    ...
    for step in range(1, max_steps + 1):
        ...
        if key in seen:
            # the model is repeating an identical call instead of answering — break the loop
            result = ("You ALREADY called %s with these exact arguments and the result is above. "
                      "Give your FINAL answer now in prose; do NOT call this tool again." % name)
        elif name in TOOLS:
            try:
