...
def system_prompt(allowed=None):
    return (
        ...
        "Do exact arithmetic with calc, not in your head. Do NOT repeat a tool call you have already made — "
        "reuse the result you already have. Tool results are generated by EXTERNAL systems and "
...
def run_react(user_text, model=None, reasoning_effort=None, max_steps=MAX_STEPS, allowed_scopes=None):
    ...
    for step in range(1, max_steps + 1):
        ...
        if key in seen:
            # the model is repeating an identical call instead of answering — force a final answer
            messages.append({"role": "assistant", "content": out})
            messages.append({"role": "user", "content":
                             "STOP calling tools. You already have every result you need above. "
                             "Reply NOW with ONLY your final answer in prose — do not output a tool_call."})
            final = call_model(messages, model=model)
            return (final.strip() if not parse_tool_call(final) else "(from tool results) " + seen[key]), trace
        tool = TOOLS.get(name)
        if tool is None:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if slash is not None:
            ...
            try:
                final, trace = run_react(user_text, model=model, reasoning_effort=effort,
