...
- If your manifest is a new file, it must be referenced by a sibling `kustomization.yaml` or Flux never applies it. `litellm.yaml` and `hermes.yaml` are already referenced.
